...
@section('title', 'Daftar Pengajuan - DISKOMINFOPERSANTIK')
...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-12 d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div>
                <h4 class="mb-1 fw-bold">Daftar Pengajuan</h4>
                <p class="text-muted mb-0" style="font-size:13px;">
                    Manajemen tiket permohonan layanan dari pengguna
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        @php
            $totalAll = $requests->count();
            $totalSent = $requests->where('status', 'sent')->count();
            $totalProgress = $requests->where('status', 'in_progress')->count();
            $totalDone = $requests->where('status', 'done')->count();
            $totalReject = $requests->where('status', 'reject')->count();
        @endphp
        <div class="col-6 col-md-3">
            <div class="stat-card">
                <div class="stat-icon" style="background:#eff6ff;">
                    <i class="ti ti-file-text" style="color:#2563eb;"></i>
                </div>
                <div>
                ...
                <table id="requestsTable" class="table table-hover align-middle" style="width:100%">
                    ...
                    <tbody>
                        @forelse ($requests as $index => $req)
                            @php
                                $status = $statuses->get($req->status);
                                $badgeCls = $status ? $status->value2 : 'secondary';
                                $label = $status ? $status->label : $req->status;
                                $cat = $categories->get($req->category);
                                $catLabel = $cat ? $cat->label : $req->category;

                                // Hitung jumlah file yang diupload
                                $fileCount = collect([
                                    $req->file_surat_pengantar,
                                    $req->file_addition1,
                                    $req->file_addition2,
                                    $req->file_addition3,
                                ])
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="file-indicator {{ $fileCount > 0 ? 'has-file' : '' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    ...
                                    <p class="text-muted mt-2 mb-0">Belum ada data pengajuan.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    html: `Pengajuan <strong>#${id} - ${title}</strong> akan dihapus dari daftar.`,
                    icon: 'warning',
